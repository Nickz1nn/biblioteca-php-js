...
<?php
	...
	if($qtd > 0){
		...
		while ($row = $res->fetch_object()) {
			...
			print "<td>
					<button class='btn btn-primary' onclick=\"location.href='?page=editar-atendente&id_atendente=".$row->id_atendente."';\">Editar</button>

					<button class='btn btn-danger' onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=salvar-atendente&acao=excluir&id_atendente=".$row->id_atendente."';}else{false;}\">Excluir</button>
				   </td>";
			print "</tr>";
		}
		print "</table>";
